...
int bg_team_warp(int bg_id, unsigned short mapindex, short x, short y) {
	...
	if(bgd == NULL) return 0;
	for(i = 0; i < MAX_BG_MEMBERS; i++)
		if(bgd->members[i].sd != NULL) pc_setpos(bgd->members[i].sd, mapindex, x, y, CLR_TELEPORT);
	return 1;
}
...
int bg_team_leave(struct map_session_data *sd, int flag) {
	...
	if(i < MAX_BG_MEMBERS) { // Removes member from BG
		if(sd->bg_queue.arena) {
			...
			pc_setpos(sd,bgd->members[i].source.map, bgd->members[i].source.x, bgd->members[i].source.y, CLR_OUTSIGHT);
		}
...
int bg_send_xy_timer_sub(DBKey key, DBData *data, va_list ap) {
	struct battleground_data *bgd = db_data2ptr(data);
	struct map_session_data *sd;
	int i;
	nullpo_ret(bg);
	for(i = 0; i < MAX_BG_MEMBERS; i++) {
...
{
	bg->team_db->foreach(bg->team_db, bg_send_xy_timer_sub, tick);
	return 0;
}
