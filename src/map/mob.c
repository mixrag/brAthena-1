		...
		if(class_ == MOBID_EMPERIUM && !no_guardian_data) {
			...
			if(gc) {
				...
				if(g) {
					...
					add_timer(gettick()+5000,mob->spawn_guardian_sub,md->bl.id,md->guardian_data->guild_id);
			}
...
int mob_setdelayspawn(struct mob_data *md)
{
	...
	md->spawn_timer = add_timer(gettick()+spawntime, mob->delayspawn, md->bl.id, 0);
	return 0;
}
	...
	if(md->spawn) {  //Respawn data
		...
		if((md->bl.x == 0 && md->bl.y == 0) || md->spawn->xs || md->spawn->ys) {
			...
			if(!map_search_freecell(&md->bl, -1, &md->bl.x, &md->bl.y, md->spawn->xs, md->spawn->ys, battle_config.no_spawn_on_player?4:0)) {
				...
				if(md->spawn_timer != INVALID_TIMER)
					delete_timer(md->spawn_timer, mob->delayspawn);
				md->spawn_timer = add_timer(tick+5000,mob->delayspawn,md->bl.id,0);
				return 1;
			}
		} else if(battle_config.no_spawn_on_player > 99 && map_foreachinrange(mob->count_sub, &md->bl, AREA_SIZE, BL_PC)) {
...
int mob_warpchase(struct mob_data *md, struct block_list *target)
{
	...
	map_foreachinrange(mob->warpchase_sub, &md->bl,
...
bool mob_ai_sub_hard(struct mob_data *md, int64 tick) {
	...
	if(md->attacked_id && mode&MD_CANATTACK) {
		...
		} else if((abl = map_id2bl(md->attacked_id)) && (!tbl || mob->can_changetarget(md, abl, mode))) {
			int dist;
	...
	if(!tbl && mode&MD_LOOTER && md->lootitem && DIFF_TICK(tick, md->ud.canact_tick) > 0 &&
	   (md->lootitem_count < LOOTITEM_SIZE || battle_config.monster_loot_type != 1)) {
		...
		map_foreachinrange(mob->ai_sub_hard_lootsearch, &md->bl, view_range, BL_ITEM, md, &tbl);
	}

	if((!tbl && mode&MD_AGGRESSIVE) || md->state.skillstate == MSS_FOLLOW) {
		map_foreachinrange(mob->ai_sub_hard_activesearch, &md->bl, view_range, DEFAULT_ENEMY_TYPE(md), md, &tbl, mode);
	} else if(mode&MD_CHANGECHASE && (md->state.skillstate == MSS_RUSH || md->state.skillstate == MSS_FOLLOW)) {
		int search_size;
		search_size = view_range<md->status.rhw.range ? view_range:md->status.rhw.range;
		map_foreachinrange(mob->ai_sub_hard_changechase, &md->bl, search_size, DEFAULT_ENEMY_TYPE(md), md, &tbl);
	}
	...
	if(!tbl) {  //No targets available.
		...
		if(md->bg_id && mode&MD_CANATTACK) {
			...
			map_foreachinrange(mob->ai_sub_hard_bg_ally, &md->bl, view_range, BL_PC, md, &tbl, mode);
			if(tbl) {
...
int mob_ai_sub_foreachclient(struct map_session_data *sd,va_list ap)
{
	int64 tick;
	tick=va_arg(ap,int64);
	map_foreachinrange(mob->ai_sub_hard_timer,&sd->bl, AREA_SIZE+ACTIVE_AI_RANGE, BL_MOB,tick);
...
int mob_ai_lazy(int tid, int64 tick, int id, intptr_t data) {
	map_foreachmob(mob->ai_sub_lazy,tick);
	return 0;
}
...
int mob_ai_hard(int tid, int64 tick, int id, intptr_t data) {
	...
	if(battle_config.mob_ai&0x20)
		...
		map_foreachpc(mob->ai_sub_foreachclient,tick);
...
int mob_deleteslave(struct mob_data *md)
{
	...
	map_foreachinmap(mob->deleteslave_sub, md->bl.m, BL_MOB,md->bl.id);
	return 0;
}
...
void mob_damage(struct mob_data *md, struct block_list *src, int damage) {
	...
	if(md->special_state.ai == 2) {  //LOne WOlf explained that ANYONE can trigger the marine countdown skill. [Skotlex]
		...
		mob->skill_use(md, gettick(), MSC_ALCHEMIST);
	}
}
...
int mob_dead(struct mob_data *md, struct block_list *src, int type) {
	...
	if(!(type&1) && !map[m].flag.nomobloot && !md->state.rebirth && (
		...
		if(sd == mvp_sd && pc_checkskill(sd,BS_FINDINGORE)>0 && battle_config.finding_ore_rate/10 >= rnd()%10000) {
			ditem = mob->setdropitem(itemdb_searchrandomid(IG_FINDINGORE), 1, NULL);
			mob->item_drop(md, dlist, ditem, 0, battle_config.finding_ore_rate / 10, homkillonly);
		}
		...
		if(dlist->item)  //There are drop items.
			add_timer(tick + (!battle_config.delay_battle_damage?500:0), mob->delay_item_drop, 0, (intptr_t)dlist);
		else //No drops
...
int mob_warpslave(struct block_list *bl, int range) {
	...
	return map_foreachinmap(mob->warpslave_sub, bl->m, BL_MOB, bl, range);
}
...
int mob_countslave(struct block_list *bl) {
	return map_foreachinmap(mob->countslave_sub, bl->m, BL_MOB,bl->id);
}
...
struct block_list *mob_getfriendhprate(struct mob_data *md,int min_rate,int max_rate) {
	...
	map_foreachinrange(mob->getfriendhprate_sub, &md->bl, 8, type,md,min_rate,max_rate,&fr);
	return fr;
}
/*==========================================
...
struct mob_data *mob_getfriendstatus(struct mob_data *md,int cond1,int cond2) {
	...
	map_foreachinrange(mob->getfriendstatus_sub, &md->bl, 8,BL_MOB, md,cond1,cond2,&fr);
	return fr;
}
...
int mob_clone_spawn(struct map_session_data *sd, int16 m, int16 x, int16 y, const char *event, int master_id, int mode, int flag, unsigned int duration) {
	...
	for(i=0,j = MAX_SKILL_TREE-1; j>=0 && i< MAX_MOBSKILL ; j--) {
		 ...
		 **/
		if(!skill_check_condition_castbegin(sd,skill_id,sd->status.skill[idx].lv))
			continue;
	...
	if(master_id || flag || duration) {  //Further manipulate crafted char.
		...
		if(duration) { //Auto Delete after a while.
			if(md->deletetimer != INVALID_TIMER)
				delete_timer(md->deletetimer, mob->timer_delete);
			md->deletetimer = add_timer(gettick() + duration, mob->timer_delete, md->bl.id, 0);
		}
	}
...
bool mob_parse_row_mobskilldb(char **str, int columns, int current)
{
	...
	if(skill_get_casttype2(sidx) == CAST_GROUND) {//Ground skill
		//Ground skill.
 ...
 */
void mob_load(void) {
	sv_readsqldb(get_database_name(39), NULL, 2+MAX_ITEMRATIO_MOBS, -1, mob->readdb_itemratio);
	mob->readchatdb();
	mob->read_sqldb();
	mob->read_sqlskilldb();
	sv_readsqldb(get_database_name(40), NULL, 12, -1, mob->readdb_mobavail);
	mob->read_randommonster();
	sv_readsqldb(get_database_name(46), NULL, 20, -1, mob->readdb_race2);
}
...
int do_init_mob(void) {
	...
	add_timer_func_list(mob->delayspawn,"mob_delayspawn");
	add_timer_func_list(mob->delay_item_drop,"mob_delay_item_drop");
	add_timer_func_list(mob->ai_hard,"mob_ai_hard");
	add_timer_func_list(mob->ai_lazy,"mob_ai_lazy");
	add_timer_func_list(mob->timer_delete,"mob_timer_delete");
	add_timer_func_list(mob->spawn_guardian_sub,"mob_spawn_guardian_sub");
	add_timer_func_list(mob->respawn,"mob_respawn");
	add_timer_interval(gettick()+MIN_MOBTHINKTIME,mob->ai_hard,0,0,MIN_MOBTHINKTIME);
	add_timer_interval(gettick()+MIN_MOBTHINKTIME*10,mob->ai_lazy,0,0,MIN_MOBTHINKTIME*10);
