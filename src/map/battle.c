...
struct block_list *battle_gettargeted(struct block_list *target) {
	...
	map->foreachinrange(battle_gettargeted_sub, target, AREA_SIZE, BL_CHAR, bl_list, &c, target->id);
	if(c == 0)
		return NULL;
	if(c > 24)
		c = 24;
...
struct block_list *battle_getenemy(struct block_list *target, int type, int range) {
	...
	memset(bl_list, 0, sizeof(bl_list));
	map->foreachinrange(battle_getenemy_sub, target, range, type, bl_list, &c, target);
...
struct block_list *battle_getenemyarea(struct block_list *src, int x, int y, int range, int type, int ignore_id) {
	...
	memset(bl_list, 0, sizeof(bl_list));
	map->foreachinarea(battle_getenemyarea_sub, src->m, x - range, y - range, x + range, y + range, type, bl_list, &c, src, ignore_id);
	...
	if(target && target->type == BL_SKILL) {
		if(atk_elem == ELE_FIRE && battle_getcurrentskill(target) == GN_WALLOFTHORN) {
			...
			if(sg->unit_id != UNT_FIREWALL) {
				...
				x = sg->val3 >> 16;
				y = sg->val3 & 0xffff;
				skill_unitsetting(sgsrc,su->group->skill_id,su->group->skill_lv,x,y,1);
				sg->val3 = -1;
...
int64 battle_calc_weapon_damage(struct block_list *src, struct block_list *bl, uint16 skill_id, uint16 skill_lv, struct weapon_atk *watk, int nk, bool n_ele, short s_ele, short s_ele_, int size, int type, int flag, int flag2){ // [malufett]
	...
	if(sd) {
		if(type == EQI_HAND_R)
			damage = battle_calc_sizefix(sd, damage, EQI_HAND_R, size, flag&8);
		else
			damage = battle_calc_sizefix(sd, damage, EQI_HAND_L, size, flag&8);
	...
	// Temporary. [malufett]
	damage = battle_calc_elefix(src, bl, skill_id, skill_lv, damage + eatk, nk, n_ele, s_ele, s_ele_, type == EQI_HAND_L, flag);
	...
	**/
	...
	// attacker side
	damage = battle_calc_cardfix(BF_WEAPON, src, bl, nk, s_ele, s_ele_, damage, 2|(type == EQI_HAND_L), flag2);

	// target side
	damage = battle_calc_cardfix(BF_WEAPON, src, bl, nk, s_ele, s_ele_, damage, 0, flag2);
...
int64 battle_calc_base_damage(struct block_list *src, struct block_list *bl, uint16 skill_id, uint16 skill_lv, int nk, bool n_ele, short s_ele, short s_ele_, int type, int flag, int flag2)
{
	...
	batk = battle_calc_elefix(src, bl, skill_id, skill_lv, status->calc_batk(bl, status->get_sc(src), st->batk, false), nk, n_ele, ELE_NEUTRAL, ELE_NEUTRAL, false, flag);

	if(type == EQI_HAND_L)
		damage = batk + 3 * battle_calc_weapon_damage(src, bl, skill_id, skill_lv, &st->lhw, nk, n_ele, s_ele, s_ele_, status_get_size(bl), type, flag, flag2) / 4;
	else
...
int64 battle_addmastery(struct map_session_data *sd,struct block_list *target,int64 dmg,int type)
{
	...
	if((skill_lv = pc_checkskill(sd, RA_RANGERMAIN)) > 0 && (st->race == RC_BRUTE || st->race == RC_PLANT || st->race == RC_FISH))
		damage += (skill_lv * 5);
	if((skill_lv = pc_checkskill(sd,NC_RESEARCHFE)) > 0 && (st->def_ele == ELE_FIRE || st->def_ele == ELE_EARTH))
		damage += (skill_lv * 10);
	if(pc_ismadogear(sd))
	...
	if((skill_lv = pc_checkskill(sd,BS_WEAPONRESEARCH)) > 0)
		damage += (skill_lv * 2);
#endif
	if((skill_lv = pc_checkskill(sd,HT_BEASTBANE)) > 0 && (st->race==RC_BRUTE || st->race==RC_INSECT)) {
		damage += (skill_lv * 4);
...
int64 battle_calc_masteryfix(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int64 damage, int div, bool left, bool weapon){
	...
	damage = battle_addmastery(sd, target, damage, left);
	...
	switch(skill_id) { // specific skill masteries
		...
		case RA_WUGSTRIKE:
		case RA_WUGBITE:	
			damage += 30*pc_checkskill(sd, RA_TOOTHOFWUG);
			break;
		case HT_FREEZINGTRAP:
			damage += 40 * pc_checkskill(sd, RA_RESEARCHTRAP);
			break;
	}
...
int64 battle_calc_elefix(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int64 damage, int nk, int n_ele, int s_ele, int s_ele_, bool left, int flag){
	...
	if(damage > 0) {
		if(left)
			damage = battle_attr_fix(src, target, damage, s_ele_, tstatus->def_ele, tstatus->ele_lv);
		else{
			...
			if(skill_id == MC_CARTREVOLUTION) //Cart Revolution applies the element fix once more with neutral element
				damage = battle_attr_fix(src,target,damage,ELE_NEUTRAL,tstatus->def_ele, tstatus->ele_lv);
...
#if VERSION != 1
	{
		...
		if(sc && sc->data[SC_SUB_WEAPONPROPERTY]) { // Descriptions indicate this means adding a percent of a normal attack in another element. [Skotlex]
			int64 temp = battle_calc_base_damage2(sstatus, &sstatus->rhw, sc, tstatus->size, BL_CAST(BL_PC, src), (flag?2:0)) * sc->data[SC_SUB_WEAPONPROPERTY]->val2 / 100;
			damage += battle_attr_fix(src, target, temp, sc->data[SC_SUB_WEAPONPROPERTY]->val1, tstatus->def_ele, tstatus->ele_lv);
			if(left) {
				temp = battle_calc_base_damage2(sstatus, &sstatus->lhw, sc, tstatus->size, BL_CAST(BL_PC, src), (flag?2:0)) * sc->data[SC_SUB_WEAPONPROPERTY]->val2 / 100;
				damage += battle_attr_fix(src, target, temp, sc->data[SC_SUB_WEAPONPROPERTY]->val1, tstatus->def_ele, tstatus->ele_lv);
			}
		}
	}
			...
			switch(skill_id){
				...
				case WL_COMET:
					...
					if( sd && sd->status.party_id ){
						...
						if( (psd = map->id2sd(p_sd[c])) && pc_checkskill(psd,WL_COMET) > 0 ){
				...
				case SO_ELECTRICWALK:
					...
					break;
				case SO_EARTHGRAVE:
					skillratio = st->int_ * skill_lv + 200 * (sd ? pc_checkskill(sd,SA_SEISMICWEAPON) : 1);
					RE_LVL_DMOD(100);
					if( sc && sc->data[SC_CURSED_SOIL_OPTION] )
						skillratio += sc->data[SC_CURSED_SOIL_OPTION]->val3 * 5;
					break;
				case SO_DIAMONDDUST:
					skillratio = (st->int_ * skill_lv + 200 * (sd ? pc_checkskill(sd, SA_FROSTWEAPON) : 1)) * status->get_lv(src) / 100;
			...
			switch( skill_id )
			{
				...
				case RK_HUNDREDSPEAR:
						...
						if( sd ){
							...
							skillratio = skillratio * (100 + (status->get_lv(src)-100) / 2) / 100 + 50 * pc_checkskill(sd,LK_SPIRALPIERCE);
						}
					break;
				case RK_WINDCUTTER:
				...
				case RK_CRUSHSTRIKE:
					...
					break;
				case RK_STORMBLAST:
					skillratio = ((sd ? pc_checkskill(sd,RK_RUNEMASTERY) : 1) + status_get_int(src) / 8) * 100;
					break;
				...
				case RA_WUGDASH:// ATK 300%
					...
					if(sc && sc->data[SC_DANCE_WITH_WUG])
						skillratio += 10 * sc->data[SC_DANCE_WITH_WUG]->val1 * (2 + battle_calc_chorusbonus(sd));
					break;
				case RA_WUGSTRIKE:
					skillratio = 200 * skill_lv;
					if( sc && sc->data[SC_DANCE_WITH_WUG] )
				...
				case LG_MOONSLASHER:
					...
					break;
				case LG_OVERBRAND:
					skillratio += -100 + 50 * (((sd) ? pc_checkskill(sd,CR_SPEARQUICKEN) : 1) + 8 * skill_lv);
					RE_LVL_DMOD(150);
					break;
				...
				case GN_CART_TORNADO:
				{
					...
					skillratio = 50 * skill_lv + (sd ? sd->cart_weight : battle_config.max_cart_weight) / 10 / (150 - strbonus) + 50 * (sd ? pc_checkskill(sd, GN_REMODELING_CART) : 5);
				}
					break;
				case GN_CARTCANNON:
					skillratio = 50 * (sd ? pc_checkskill(sd, GN_REMODELING_CART) : 5) * (st->int_ / 40) + 60 * skill_lv;
					break;
				...
				case KO_SETSUDAN:
					...
					break;
				case KO_BAKURETSU:
					skillratio += -100 + (50 + status_get_dex(src) / 4) * skill_lv * (sd?pc_checkskill(sd,NJ_TOBIDOUGU):10) * 4 / 100;
					RE_LVL_DMOD(120);
...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv) {
	...
	if(sc && sc->count) {
		...
		if(sc->data[SC_HOVERING] && skill_id && (skill_get_inf(skill_id)&INF_GROUND_SKILL || skill_id == SR_WINDMILL)) {
			d->dmg_lv = ATK_BLOCK;
			return 0;
		}
		if((sce=sc->data[SC_AUTOGUARD]) && flag&BF_WEAPON && !(skill_get_nk(skill_id)&NK_NO_CARDFIX_ATK) && rnd()%100 < sce->val2)
{
		...
		if( (sce = sc->data[SC_STONEHARDSKIN]) && flag&(BF_SHORT|BF_WEAPON) && damage > 0 ) {
			...
			if( src->type == BL_MOB )
				sc_start(bl, src, SC_NOEQUIPWEAPON, 30, 0, skill_get_time2(RK_STONEHARDSKIN, sce->val1));
			if( sce->val2 <= 0 )
		...
		if (sc->data[SC_STYLE_CHANGE]) {
                    ...
                    if (hd && (rnd()%100<(status->get_lv(bl)/2))) homun->addspiritball(hd, 10); //add a sphere
                }
	...
	if(sc && sc->count) {
		...
		if(tsc->data[SC_POISONINGWEAPON]) {
			...
			if (!(flag&BF_SKILL) && (flag&BF_WEAPON) && damage > 0 && rnd()%100 < tsc->data[SC_POISONINGWEAPON]->val3 ) {
				...
				sc_start(src,bl,tsc->data[SC_POISONINGWEAPON]->val2,rate,tsc->data[SC_POISONINGWEAPON]->val1,skill_get_time2(GC_POISONINGWEAPON,1) - (tstatus->vit + tstatus->luk) / 2 * 1000);
			}
		}
		if(sc->data[SC__DEADLYINFECT] && flag&BF_SHORT && damage > 0 && rnd()%100 < 30 + 10 * sc->data[SC__DEADLYINFECT]->val1 && !is_boss(src))
			status->change_spread(src, bl);
		if(sc->data[SC_SHIELDSPELL_REF] && sc->data[SC_SHIELDSPELL_REF]->val1 == 1 && damage > 0)
			skill_break_equip(bl,EQP_ARMOR,10000,BCT_ENEMY);
...
void battle_consume_ammo(TBL_PC *sd, int skill_id, int lv) {
	...
	if(skill_id) {
		qty = skill_get_ammo_qty(skill_id, lv);
		if(!qty) qty = 1;
	}
...
#define MATK_ADD( a ) ( ad.damage+= (a) )
		...
		{	//Calc base damage according to skill
			...
			case AB_HIGHNESSHEAL:
				ad.damage = skill_calc_heal(src, target, AL_HEAL, 10, false) * ( 17 + 3 * skill_lv ) / 10;
				break;
			case PR_ASPERSIO:
				ad.damage = 40;
				break;
            ...
            default: {
				...
				switch(skill_id){
					...
					default:
						MATK_RATE(battle_calc_skillratio(BF_MAGIC, src, target, skill_id, skill_lv, skillratio, mflag));
				}
				//Constant/misc additions from skills
				if (skill_id == WZ_FIREPILLAR)
					MATK_ADD(50);
				if(sd && (sd->status.class_ == JOB_ARCH_BISHOP_T || sd->status.class_ == JOB_ARCH_BISHOP) &&
					(i=pc_checkskill(sd,AB_EUCHARISTICA)) > 0 &&
					(tstatus->race == RC_DEMON || tstatus->def_ele == ELE_DARK) )
					MATK_ADDRATE(i);
			}
		}
		...
		ad.damage = battle_calc_defense(BF_MAGIC, src, target, skill_id, skill_lv, ad.damage, (flag.imdef?1:0), 0);
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag) {
	...
	case SN_FALCONASSAULT:
		...
		if (skill_id == SN_FALCONASSAULT) {
			...
			temp = skill_get_num(HT_BLITZBEAT, 5);
	...
	case CR_ACIDDEMONSTRATION: 
		...
		{// [malufett]
			...
			matk = battle_calc_magic_attack(src, target, skill_id, skill_lv, mflag).damage;
			atk = battle_calc_base_damage(src, target, skill_id, skill_lv, nk, false, s_ele, ELE_NEUTRAL, EQI_HAND_R, (sc && sc->data[SC_MAXIMIZEPOWER]?1:0)|(sc && sc->data[SC_WEAPONPERFECT]?8:0), md.flag);
			md.damage = matk + atk;
			if( src->type == BL_MOB ){
	...
	case KO_MUCHANAGE:
		...
		if ( is_boss(target) || (sd && !pc_checkskill(sd,NJ_TOBIDOUGU)) )
			md.damage >>= 1;
		break;
	case NJ_ZENYNAGE:
		md.damage = skill_get_zeny(skill_id ,skill_lv);
	...
	case ASC_BREAKER:
		...
		int64 matk = battle_calc_magic_attack(src, target, skill_id, skill_lv, mflag).damage;
		short totaldef = status->get_total_def(target) + status->get_total_mdef(target);
		int64 atk = battle_calc_base_damage(src, target, skill_id, skill_lv, nk, false, s_ele, ELE_NEUTRAL, EQI_HAND_R, (sc && sc->data[SC_MAXIMIZEPOWER]?1:0)|(sc && sc->data[SC_WEAPONPERFECT]?8:0), md.flag);
	...
	case WM_SOUND_OF_DESTRUCTION:
		...
		md.damage += md.damage * 10 * battle_calc_chorusbonus(sd) / 100;
		break;
	/**
	 * Mechanic
	 **/
	...
	case GN_HELLS_PLANT_ATK:
		md.damage = skill_lv * status->get_lv(src) * 10 + sstatus->int_ * 7 / 2 * (18 + (sd ? sd->status.job_level : 0) / 4) * (5 / (10 - (sd ? pc_checkskill(sd, AM_CANNIBALIZE) : 0)));
...
#if VERSION == 1
#define GET_NORMAL_ATTACK( f ) ( wd.damage = battle_calc_base_damage(src, target, skill_id, skill_lv, nk, n_ele, s_ele, s_ele_, EQI_HAND_R, (f), wd.flag) )
#define GET_NORMAL_ATTACK2( f ) ( wd.damage2 = battle_calc_base_damage(src, target, skill_id, skill_lv, nk, n_ele, s_ele, s_ele_, EQI_HAND_L, (f), wd.flag) )
#endif
		switch(skill_id) {
				//Calc base damage according to skill
			case PA_SACRIFICE:
				...
				wd.damage2 = 0;
#if VERSION == 1
				wd.damage = battle_calc_elefix(src, target, skill_id, skill_lv, wd.damage, nk, n_ele, s_ele, s_ele_, false, wd.flag); // temporary [malufett]
#endif
				break;
			...
			case NJ_SYURIKEN: // [malufett]
				GET_NORMAL_ATTACK((sc && sc->data[SC_MAXIMIZEPOWER]?1:0)|(sc && sc->data[SC_WEAPONPERFECT]?8:0));
				wd.damage += battle_calc_masteryfix(src, target, skill_id, skill_lv, 4 * skill_lv + (sd ? sd->bonus.arrow_atk : 0), wd.div_, 0, flag.weapon) - status->get_total_def(target);
				RE_SKILL_REDUCTION();
				break;
			...
			case ML_SPIRALPIERCE:
				if(sd) {
					...
					wd.damage = battle_calc_base_damage2(sstatus, &sstatus->rhw, sc, tstatus->size, sd, 0); //Monsters have no weight and use ATK instead
				}
				i = sstatus->str/10;
				i*=i;
...
#if VERSION == 1
				GET_NORMAL_ATTACK(i);
				wd.damage = battle_calc_masteryfix(src, target, skill_id, skill_lv, wd.damage, wd.div_, 0, flag.weapon);
				if (flag.lh){
					GET_NORMAL_ATTACK2(i);
					wd.damage2 = battle_calc_masteryfix(src, target, skill_id, skill_lv, wd.damage2, wd.div_, 1, flag.weapon);
				}
#else
				wd.damage = battle_calc_base_damage2(sstatus, &sstatus->rhw, sc, tstatus->size, sd, i);
				if (flag.lh)
					wd.damage2 = battle_calc_base_damage2(sstatus, &sstatus->lhw, sc, tstatus->size, sd, i);
#endif
				if (nk&NK_SPLASHSPLIT){ // Divide ATK among targets
					if(wflag>0)
						wd.damage/= wflag;
		...
		if(sc && !skill_id && sc->data[SC_EXEEDBREAK]) {
			...
			switch (skill_id) {
#if VERSION == 1
				case HW_MAGICCRASHER:
					ATK_ADD(battle_calc_magic_attack(src, target, skill_id, skill_lv, wflag).damage / 5);
					break;
#else
				case MO_EXTREMITYFIST:
				...
				case RA_WUGDASH:
					if(sc && sc->data[SC_DANCE_WITH_WUG])
						ATK_ADD(2 * sc->data[SC_DANCE_WITH_WUG]->val1 * (2 + battle_calc_chorusbonus(sd)));
					break;
				case SR_TIGERCANNON:
		...
		if(!flag.idef || !flag.idef2) { //Defense reduction
			wd.damage = battle_calc_defense(BF_WEAPON, src, target, skill_id, skill_lv, wd.damage,
				...
				, flag.pdef);
			if( wd.damage2 )
				wd.damage2 = battle_calc_defense(BF_WEAPON, src, target, skill_id, skill_lv, wd.damage2,
...
#if VERSION != 1
	wd.damage = battle_calc_elefix(src, target, skill_id, skill_lv, wd.damage, nk, n_ele, s_ele, s_ele_, false, flag.arrow);
	if(flag.lh)
		wd.damage2 = battle_calc_elefix(src, target, skill_id, skill_lv, wd.damage2, nk, n_ele, s_ele, s_ele_, true, flag.arrow);
#endif
    ...
    if(tsd){ //if player on player then it was already measured above
		wd.damage = battle_calc_cardfix(BF_WEAPON, src, target, nk, s_ele, s_ele_, wd.damage, (flag.lh?1:0), wd.flag);
	}
#endif
	...
	if(sd) {
		if(!flag.rh && flag.lh) {   //Move lh damage to the rh
			...
			if(wd.damage) {
				skill = pc_checkskill(sd,AS_RIGHT) * 10;
				if((sd->class_&MAPID_UPPERMASK) == MAPID_KAGEROUOBORO )
					skill = pc_checkskill(sd,KO_RIGHT) * 10 + 20;
						ATK_RATER(50 + skill);
			}
			if(wd.damage2) {
				skill = pc_checkskill(sd,AS_LEFT) *10;
					if((sd->class_&MAPID_UPPERMASK) == MAPID_KAGEROUOBORO)
	...
	if(wd.damage + wd.damage2) { //There is a total damage value
		...
		if(src != target) { // Don't reflect your own damage (Grand Cross)
			...
			if(wd.dmg_lv == ATK_MISS || wd.dmg_lv == ATK_BLOCK) {
				...
				battle_reflect_damage(target, src, &wd, skill_id);
	...
	if(wd.damage && tsc && tsc->data[SC_SWORDREJECT] &&
	   ...
	   rnd()%100 < tsc->data[SC_SWORDREJECT]->val2
		...
		ATK_RATER(50);
		status_fix_damage(target,src,wd.damage,clif_damage(target,src,0,0,wd.damage,0,0,0));
		clif_skill_nodamage(target,target,ST_REJECTSWORD,tsc->data[SC_SWORDREJECT]->val1,1);
			...
			if(!is_boss(src)) {
				if( sc->data[SC_DEATHBOUND] && skill_id != WS_CARTTERMINATION) {
					...
					if(!map->check_dir(dir,t_dir)) {
						...
						rdelay = clif_skill_damage(src, target, tick, status_get_amotion(src), status_get_dmotion(src), -3000, 1, RK_DEATHBOUND, sc->data[SC_DEATHBOUND]->val1, 6);
						...
						if( tsd ) /* is this right? rdamage as both left and right? */
							battle_drain(tsd, src, rdamage, rdamage, status_get_race(src), 0);
		...
		if(wd->dmg_lv >= ATK_BLOCK ) {/* yes block still applies, somehow gravity thinks it makes sense. */
			if(sc) {
				...
				if(sc->data[SC_LG_REFLECTDAMAGE] && rand()%100 < (30 + 10*sc->data[SC_LG_REFLECTDAMAGE]->val1)) {
					...
					map->foreachinshootrange(battle_damage_area,target,skill_get_splash(LG_REFLECTDAMAGE,1),BL_CHAR,tick,target,delay,wd->dmotion,rdamage,status_get_race(target));
			...
			if((ssc = status->get_sc(src))) {
				if(ssc->data[SC_INSPIRATION]) {
					...
					/* is this right? rdamage as both left and right? */
					if(sd)
						battle_drain(sd, target, rdamage, rdamage, status_get_race(target), 0);
					battle_delay_damage(tick, wd->amotion,src,target,0,CR_REFLECTSHIELD,0,rdamage,ATK_DEF,rdelay,true);
...
int battle_damage_area(struct block_list *bl, va_list ap)
{
	...
	if(bl != src && battle_check_target(src,bl,BCT_ENEMY) > 0) {
		...
		clif_damage(bl,bl,amotion,dmotion,damage,1,ATK_BLOCK,0);
...
enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, int64 tick, int flag)
{
	...
	if(tsc && tsc->data[SC_AUTOCOUNTER] && status->check_skilluse(target, src, KN_AUTOCOUNTER, 1)) {
		...
		if(dist <= 0 || (!map->check_dir(dir,t_dir) && dist <= tstatus->rhw.range+1)) {
			...
			clif_damage(src, target, sstatus->amotion, 1, 0, 1, 0, 0); //Display MISS.
	...
	if(tsc && tsc->data[SC_BLADESTOP_WAIT] && !is_boss(src) && (src->type == BL_PC || tsd == NULL || distance_bl(src, target) <= (tsd->status.weapon == W_FIST ? 1 : 2))) {
		...
		if(sc_start4(target, src, SC_BLADESTOP, 100, sd?pc_checkskill(sd, MO_BLADESTOP):5, 0, 0, target->id, duration)) {
			//Target locked.
			clif_damage(src, target, sstatus->amotion, 1, 0, 1, 0, 0); //Display MISS.
	...
	if(sc) {
		...
		if(sc->data[SC_GENTLETOUCH_ENERGYGAIN]) {
			if(sd && rnd()%100 < 10 + 5 * sc->data[SC_GENTLETOUCH_ENERGYGAIN]->val1)
				pc_addspiritball(sd,
				                 skill_get_time(MO_CALLSPIRITS, sc->data[SC_GENTLETOUCH_ENERGYGAIN]->val1),
		...
		if(tsc && tsc->data[SC_GENTLETOUCH_ENERGYGAIN]) {
			if(tsd && rnd()%100 < 10 + 5 * tsc->data[SC_GENTLETOUCH_ENERGYGAIN]->val1)
				pc_addspiritball(tsd,
				                 skill_get_time(MO_CALLSPIRITS, tsc->data[SC_GENTLETOUCH_ENERGYGAIN]->val1),
	...
	if(tsc && tsc->data[SC_KAAHI] && tsc->data[SC_KAAHI]->val4 == INVALID_TIMER && tstatus->hp < tstatus->max_hp)
		tsc->data[SC_KAAHI]->val4 = add_timer(tick + skill_get_time2(SL_KAAHI,tsc->data[SC_KAAHI]->val1), status->kaahi_heal_timer, target->id, SC_KAAHI); //Activate heal.
	...
	wd.dmotion = clif_damage(src, target, wd.amotion, wd.dmotion, wd.damage, wd.div_ , wd.type, wd.damage2);
	...
	if(tsc) {
		if(tsc->data[SC_DEVOTION]) {
			...
			if(e_bl && !status->isdead(e_bl)) {
				clif_damage(e_bl,e_bl,wd.amotion,wd.dmotion,damage,wd.div_,wd.type,wd.damage2);
				status->damage(target, e_bl, damage, 0, 0, 0);
				// Just show damage in target.
				clif_damage(src, target, wd.amotion, wd.dmotion, damage, wd.div_, wd.type, wd.damage2);
				map->freeblock_unlock();
				return ATK_NONE;
			}
		}
	...
	if(sd) {
		if(wd.flag&BF_SHORT && sc && sc->data[SC__AUTOSHADOWSPELL] && rnd()%100 < sc->data[SC__AUTOSHADOWSPELL]->val3 &&
		   sd->status.skill[skill_get_index(sc->data[SC__AUTOSHADOWSPELL]->val1)].id != 0 && sd->status.skill[skill_get_index(sc->data[SC__AUTOSHADOWSPELL]->val1)].flag == SKILL_FLAG_PLAGIARIZED ) {
			int r_skill = sd->status.skill[skill_get_index(sc->data[SC__AUTOSHADOWSPELL]->val1)].id,
...
int battle_check_target(struct block_list *src, struct block_list *target,int flag)
{
		...
		case BL_PC: {
				...
				if(s_bl != t_bl) {
					if(sd->state.killer) {
						...
						strip_enemy = 0;
					} else if(sd->duel_group && !((!battle_config.duel_allow_pvp && map->list[m].flag.pvp) || (!battle_config.duel_allow_gvg && map_flag_gvg(m)))) {
	...
	if(map_flag_vs(m)) {
		...
		if(flag&(BCT_GUILD|BCT_ENEMY)) {
			...
			if(!(map->list[m].flag.pvp && map->list[m].flag.pvp_noguild) && s_guild && t_guild && (s_guild == t_guild || (!(flag&BCT_SAMEGUILD) && guild->isallied(s_guild, t_guild))) && (!map->list[m].flag.battleground || sbg_id == tbg_id))
				state |= BCT_GUILD;
			else
				state |= BCT_ENEMY;
		}
...
// BattleGround Settings
	...
	{ "skill_trap_type_ot",                 &battle_config.skill_trap_type_ot,              1,      0,      1,              },
...
#ifndef STATS_OPT_OUT
	add_timer_func_list(brAthena_report_timer, "brAthena_report_timer");
	add_timer_interval(gettick()+30000, brAthena_report_timer, 0, 0, 60000 * 30);
#endif
