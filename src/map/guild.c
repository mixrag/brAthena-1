...
TBL_PC *guild_sd_check(int guild_id, int account_id, int char_id)
{
	...
	if(sd->status.guild_id != guild_id) {
		...
		intif->guild_leave(guild_id, account_id, char_id, 0, "** Clã Incompatível**");
		return NULL;
	}
...
int guild_member_added(int guild_id,int account_id,int char_id,int flag)
{
	...
	if(sd==NULL || sd->guild_invite==0) {
		...
		if(flag == 0) {
			...
			intif->guild_leave(guild_id, account_id, char_id, 0, "** Erro de Dados **");
		}
		return 0;
	}
	...
	}
	{
		...
		for(i = 0; i < guild->flags_count; i++) {
			if(guild->flags[i] && guild->flags[i]->u.scr.guild_id == guild_id) {
				clif_guild_emblem_area(&guild->flags[i]->bl);
			}
		}
	}
	return 0;
...
int guild_skillupack(int guild_id,uint16 skill_id,int account_id)
{
	...
	if(g==NULL)
		return 0;
	if(sd != NULL) {
		clif_skillup(sd,skill_id,g->skill[skill_id-GD_SKILLBASE].lv, 0);
...
int guild_reqalliance(struct map_session_data *sd,struct map_session_data *tsd)
{
	...
	if(map->agit_flag || map->agit2_flag) {   // Disable alliance creation during woe [Valaris]
		clif_displaymessage(sd->fd,msg_txt(876)); //"Alliances cannot be made during Guild Wars!"
		return 0;
	}
...
int guild_delalliance(struct map_session_data *sd,int guild_id,int flag)
{
	...
	if((!battle_config.alliance_in_woe) && (map->agit_flag || map->agit2_flag))  {     // Disable alliance breaking during woe [Valaris]
		clif_displaymessage(sd->fd,msg_txt(877)); //"Alliances cannot be broken during Guild Wars!"
		return 0;
	}
...
int guild_gm_changed(int guild_id, int account_id, int char_id)
{
	...
	if(g->member[pos].sd && g->member[pos].sd->fd) {
		clif_displaymessage(g->member[pos].sd->fd, msg_txt(878)); //"You no longer are the Guild Master."
		g->member[pos].sd->state.gmaster_flag = 0;
	}

	if(g->member[0].sd && g->member[0].sd->fd) {
		clif_displaymessage(g->member[0].sd->fd, msg_txt(879)); //"You have become the Guild Master!"
...
void do_init_guild(void) {
	...
	sv_readsqldb(get_database_name(14), NULL, 5, -1, guild->read_castledb);

	sv_readsqldb(get_database_name(19), NULL , 2+MAX_GUILD_SKILL_REQUIRE*2, -1, guild->read_guildskill_tree_db);

	add_timer_func_list(guild->payexp_timer,"guild_payexp_timer");
	add_timer_func_list(guild->send_xy_timer, "guild_send_xy_timer");
	add_timer_interval(gettick()+GUILD_PAYEXP_INVERVAL,guild->payexp_timer,0,0,GUILD_PAYEXP_INVERVAL);
	add_timer_interval(gettick()+GUILD_SEND_XY_INVERVAL,guild->send_xy_timer,0,0,GUILD_SEND_XY_INVERVAL);
}
