...
char grf_list_file[256] = "conf/grf-files.conf";
...
int find_map(char *name)
{
	...
	for(i = 0; i < header.map_count; i++) {
		if(fread(&info, sizeof(info), 1, map_cache_fp) != 1) printf("Um erro como ocorreu no fread ao ler map_cache\n");
