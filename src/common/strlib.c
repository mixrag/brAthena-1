...
int sv_parse_next(struct s_svstate *svstate)
{
	...
	if(delim == '\n' && (opt&(SV_TERMINATE_CRLF|SV_TERMINATE_LF))) {
		ShowError(read_message("Source.common.sv_parse_next"));
		return -1;// error
	}
	if(delim == '\r' && (opt&(SV_TERMINATE_CRLF|SV_TERMINATE_CR))) {
		ShowError(read_message("Source.common.sv_parse_next2"));
		return -1;// error
	}
	...
	while(state != END) {
		switch(state) {
			...
			case PARSING_C_ESCAPE: { // skip escape sequence (validates it too)
					++i;// '\\'
					if(IS_END()) {
						ShowError(read_message("Source.common.sv_parse_next3"));
						return -1;
					}
					if(str[i] == 'x') {
						// hex escape
						...
						if(IS_END() || !ISXDIGIT(str[i])) {
							ShowError(read_message("Source.common.sv_parse_next4"));
							return -1;
						}
						do {
							++i;// hex digit
						...
						ShowError(read_message("Source.common.sv_parse_next5"), str[i]);
						return -1;
					}
					state = PARSING_FIELD;
					break;
...
int sv_split(char *str, int len, int startoff, char delim, char **out_fields, int nfields, enum e_svopt opt)
{
	...
	if(end[0] == '\0') {
		...
		*out_fields = end + 1;
		...
		ShowError(read_message("Source.common.sv_split"), (unsigned char)end[0]);
		return -1;// error
	}
	++out_fields;
	--nfields;
...
size_t sv_unescape_c(char *out_dest, const char *src, size_t len)
{
	...
	for(i = 0, j = 0; i < len;) {
		if(src[i] == '\\') {
			++i;// '\\'
			if(i >= len)
				ShowWarning(read_message("Source.common.sv_unescape_c"));
			else if(src[i] == 'x') {
				// hex escape sequence
				unsigned char c = 0;
				...
				if(i >= len || !ISXDIGIT(src[i])) {
					ShowWarning(read_message("Source.common.sv_unescape_c2"));
					continue;
				}
				do {
					if(c > 0x0F && inrange) {
						ShowWarning(read_message("Source.common.sv_unescape_c3"));
						inrange = 0;
					}
				...
				if(strchr(SV_ESCAPE_C_SUPPORTED, src[i]) == NULL)
					ShowWarning(read_message("Source.common.sv_unescape_c4"), src[i]);
				switch(src[i]) {
					case 'a': out_dest[j++] = '\a'; break;
					case 'b': out_dest[j++] = '\b'; break;
...
bool sv_readdb(const char *directory, const char *filename, char delim, int mincols, int maxcols, int maxrows, bool (*parseproc)(char *fields[], int columns, int current))
{
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		if(columns < mincols) {
			ShowError(read_message("Source.common.sv_readdbs"), lines, path, columns, mincols);
			continue; // not enough columns
		}
		if(columns > maxcols) {
			ShowError(read_message("Source.common.sv_readdbs2"), lines, path, columns, maxcols);
			continue; // too many columns
		}
		if(entries == maxrows) {
			ShowError(read_message("Source.common.sv_readdbs3"), maxrows, path);
			break;
		}

		// parse this row
		if(!parseproc(fields+1, columns, entries)) {
			ShowError(read_message("Source.common.sv_readdbs4"), lines, path);
	...
	aFree(fields);
	fclose(fp);
	ShowStatus(read_message("Source.common.grfio_resourcecheck"), entries, path);
